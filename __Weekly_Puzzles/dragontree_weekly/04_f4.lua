...
if y<=-1 then -- tree trunk, faster near top
	if (z==0 and x==0) then return BROWN end
	return 
end

if y>=8 then return end -- empty space above

local v= abs(x)+abs(z) -- not using these abs() function calls would change everything but speed things up
if v>14 then return end -- empty space
...
--if y==4 then -- CrashT recommends saving time
--	if v>12 then return end -- empty space, wasnt faster
	if v<=12 and v>=4 then
		if z%8==0 or x%8==0 then
			return ORANGE
		end
	end
--	return
